...
(function () {
  ...
  async function collectBody(body) {
    ...
    if (body[Symbol.asyncIterator]) {
      ...
      // isomorphic-git often passes body: [packbuffer] (sync iterable of Uint8Arrays)
  ...
  async function* streamUploadPackResponse(res) {
    ...
    try {
      while (size < 4) {
        ...
        // WHATWG: the last read may be { done: true, value: finalChunk } — dropping value truncates the pack.
