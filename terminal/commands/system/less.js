...
import { LessLib } from './less-lib.js';
...
function showLessViewer(
  ...
  function applyStartupPattern(term) {
    ...
    helpSpan.textContent = LessLib.formatLessSearchMatchFooter(result, 0, searchResults.length);
  }
  ...
  function advanceSearchForward(times) {
    ...
    currentLine = Math.max(0, result.line - Math.floor(linesPerPage / 2));
    helpSpan.textContent = LessLib.formatLessSearchMatchFooter(result, currentSearchIndex, len);
    updateDisplay();
  }
...
async function lessHandler(terminal, args) {
...
export default {
  name: 'less',
  handler: lessHandler,
  description: 'view file contents with paging and search',
  category: 'System'
};
