...
function psHandler(terminal, args) {
  ...
  processes.forEach((proc) => {
    ...
    output += `${proc.pid.toString().padStart(5)} ${proc.parentPID.toString().padStart(5)} ${String(
      proc.uid
    ).padStart(8)} ${proc.state.padEnd(5)} ${isolated.padEnd(8)} ${proc.name}\n`;
  });
...
export default {
  name: 'ps',
  handler: psHandler,
  description: 'show running processes with isolation status',
  category: 'System'
};
