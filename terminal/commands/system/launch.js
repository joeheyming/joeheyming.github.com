...
async function launchHandler(terminal, args) {
...
Tip: You can also launch apps from desktop icons or the Apps launcher (🚀)`;
  ...
  if (!app) {
    ...
    const suggestions = availableApps
      .filter(
        (a) => a.id.toLowerCase().includes(appName) || a.shortName.toLowerCase().includes(appName)
      )
      .slice(0, 3);
...
export default {
  ...
  description: 'Launch applications or open files from the terminal',
  category: 'Apps'
};
