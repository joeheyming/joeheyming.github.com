...
async function grepHandler(terminal, args) {
  ...
  const { caseInsensitive, lineNumbers, invertMatch, noFilename, pattern, fileOperands } = parsed;
...
export default {
  name: 'grep',
  handler: grepHandler,
  description: 'search for literal PATTERN in FILEs or stdin (-i -n -v -h, --, - for stdin)',
