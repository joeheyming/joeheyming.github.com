...
(function () {
  ...
  registerCommand('ls', async (terminal, args) => {
    ...
    try {
      ...
      if (stats.type === 'file') {
        if (showDetails) {
          ...
          const modified = stats.modified ? new Date(stats.modified).toISOString().slice(0, 16).replace('T', ' ') : 'unknown';
          return `-rw-r--r-- 1 ${terminal.env.USER} ${terminal.env.USER} ${size.toString().padStart(8)} ${modified} ${targetDir.split('/').pop()}`;
        } else {
  ...
  }, 'list directory contents (-l for detailed view)', 'File System');
})();
