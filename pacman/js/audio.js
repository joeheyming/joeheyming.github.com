...
export class AudioManager {
  constructor() {
    ...
    // (that path was a dominant Search Console INP contributor).
    const warm = () => {
      this.init().catch(() => {});
    };
    ...
    // keep a one-shot fallback that kicks init without blocking the
    // click handler (init is async; we intentionally do not await).
  ...
  async loadSounds() {
    // Load every WAV in parallel. The old sequential await loop made the
    // first START click wait for five network/decode round-trips on the
