...
(function () {
  ...
  // ---- ZIP reader (CDFH parser, EOCD-locator first) ----------------------
  ...
  function findEocd(view) {
    // EOCD is at the end of the file, with up to a 64KB comment after it.
  ...
  /**
   * Parse central directory and return entry descriptors.
   */
  ...
  /**
   * Extract one or more entries from a zip archive.
   *
   * @param {Uint8Array} bytes the entire archive
   * @param {{ filter?: (name: string) => boolean, max?: number }} [opts]
  ...
  async function extractZip(bytes, opts) {
    ...
    // First, sniff for non-zip archive formats so we can give a clean error.
    const sig = signatureName(bytes);
    if (sig === 'rar' || sig === '7z') {
      throw new Error(
        'Archive is .' + sig + ' which we cannot extract in-browser. ' +
          'Download manually from moddb and use the manual upload picker.'
      );
    }
    if (sig !== 'zip' && sig !== 'unknown') {
      // Not catastrophic — proceed and let the EOCD parser reject if truly bad.
    }
    ...
    if (needsInflate) {
      try {
        ...
        throw new Error(
          'Cannot decompress DEFLATE entries (pako load failed): ' + (e.message || e)
        );
      }
    }
    ...
    for (const entry of wanted.slice(0, max)) {
      ...
      if (entry.compressionMethod === 0) {
        // Stored — copy out so callers can mutate freely.
  ...
  /**
   * Identify common archive formats from the leading bytes.
   * Returns 'zip' | 'rar' | '7z' | 'gz' | 'unknown'.
   */
