    ...
    if (!this._buffer) {
      ...
      if (needsNewBuffer) {
        if (this._autoExtendsMode === 'none') {
          throw new Error(
            'Buffer has overrun, required size ' +
              minRequired +
              ' current size ' +
              this._buffer.length
          );
    ...
    if (this._writeIndex >= this._readIndex) {
      ...
      if (endOfBufferAvailable >= length) {
        ...
        this._copyArray(
          ...
          this._buffer,
          this._writeIndex,
          endOfBufferAvailable
        );
        this._copyArray(
          arrayBuffer,
          sourceIndex + endOfBufferAvailable,
          this._buffer,
          0,
          remainingData
        );
    ...
    if (readCount > 0) {
      ...
      if (this._writeIndex >= this._readIndex) {
        ...
        if (endOfBufferAvailable >= readCount) {
          ...
          this._copyArray(
            this._buffer,
            this._readIndex + endOfBufferAvailable,
            arrayBuffer,
            destIndex + endOfBufferAvailable,
            remainingData
          );
