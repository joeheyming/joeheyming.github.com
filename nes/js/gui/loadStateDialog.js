...
(function () {
  ...
  var LoadStateDialog = function (app) {
    var that = this;
    this._app = app;
    this._contentsDiv = document.getElementById('loadStateDialog_contents');

    this._dialog = document.getElementById('loadStateDialog').nativeDialog({
      autoOpen: false,
      height: 400,
      width: 900,
      modal: true,
  ...
  LoadStateDialog.prototype.show = function (cartName, meta) {
    ...
    for (var keyIndex = 0; keyIndex < keyNames.length; ++keyIndex) {
      ...
      html +=
        "<button type='button' onclick='Gui.loadSaveDialog_onclick( \"" + slotName + '" );\'>';
      if (slot.screen) {
        html +=
          "<img src='" +
          slot.screen +
