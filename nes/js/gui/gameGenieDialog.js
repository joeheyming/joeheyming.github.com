...
(function () {
  ...
  var GameGenieDialog = function (app) {
    var that = this;
    this._app = app;
    this._contentsDiv = document.getElementById('gameGenieDialog_contents');

    this._app.connect('cartLoaded', function (cart) {
      that._onCartLoaded(cart);
    });

    this._dialog = document.getElementById('gameGenieDialog').nativeDialog({
      autoOpen: false,
    ...
    if (cart && cart._dbData && cart._dbData['gameGenieCodes']) {
      ...
      for (var i = 0; i < codesArray.length; ++i) {
        ...
        html +=
          ...
          "' value='" +
          i +
          "' onclick='Gui.gameGenieDialog_onclick( " +
          i +
          " );'><span>" +
          gg.name +
          '</span>';
  ...
  var gameGenieDialog_onclick = function (gameCodeId) {
    if (_open) {
      ...
      var jqElement = document.getElementById('gg_cb_' + gameCodeId);
      var isChecked = jqElement.checked;
      ...
      for (var j = 0; j < code['codes'][selectedSubCodeIndex].length; ++j) {
        Nes.processGameGenieCode(
          _open._app._mainboard,
          code['codes'][selectedSubCodeIndex][j],
          isChecked
        );
      }
    }
