...
(function () {
  ...
  var WebGlRenderSurface = function (canvasParent) {
    ...
    canvasParent.connect('resize', function () {
      that._onResize();
    });
  ...
  WebGlRenderSurface.prototype.loadShader = function (shaderFilename, callback) {
    ...
    this._shader.loadAndLink(shaderFilename, function () {
      ...
      that._glContext.uniform2fv(
        that._shader.getUniformLocation('rubyInputSize'),
        that._inputSizeShaderArray
      );
      that._glContext.uniform2fv(
        that._shader.getUniformLocation('rubyOutputSize'),
        that._outputSizeShaderArray
      );
      that._glContext.uniform2fv(
        that._shader.getUniformLocation('rubyTextureSize'),
        that._textureSizeShaderArray
      );

      that._glContext.uniformMatrix4fv(
        that._shader.getUniformLocation('aModelViewProjectionMatrix'),
        false,
        that._camera.getMVPMatrix()
      );
  ...
  WebGlRenderSurface.prototype.render = function (mainboard) {
    ...
    this._glContext.uniform1i(
      this._shader.getUniformLocation('rubyFrameCount'),
      mainboard.ppu.frameCounter
    );
  ...
  WebGlRenderSurface.prototype._cleanup = function () {
    ...
    if (this._offscreen8BitView) {
      this._offscreen8BitView = null;
    }

    // Cleanup WebGL resources
    if (this._glContext) {
      if (this._vertexBuffer && this._vertexBuffer.cleanup) {
        this._vertexBuffer.cleanup();
      }
      if (this._textureCoordBuffer && this._textureCoordBuffer.cleanup) {
        this._textureCoordBuffer.cleanup();
      }
      if (this._indexBuffer && this._indexBuffer.cleanup) {
        this._indexBuffer.cleanup();
      }
      if (this._texture && this._texture.cleanup) {
        this._texture.cleanup();
      }
      if (this._shader && this._shader.cleanup) {
