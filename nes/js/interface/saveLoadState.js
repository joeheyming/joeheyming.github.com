...
(function () {
  ...
  // Temporarily disable compression to test if that's causing the graphics corruption
  var compress = function (rawString) {
    console.log('📦 Saving uncompressed data (length:', rawString.length, ')');
    return rawString;
  };

  var decompress = function (rawString) {
    console.log('📦 Loading uncompressed data (length:', rawString.length, ')');
    return rawString;
  };
  ...
  var saveState = function (slotName, cartName, data, screenData) {
    if (localStorage) {
      ...
      if (screenData) {
        ...
        console.log(
          'Saved screenshot size: ' +
            slotMeta.screen.length +
            ' (uncompressed: ' +
            screenData.length +
            ')'
        );
  ...
  var loadState = function (slotName, cartName) {
    if (localStorage) {
      ...
      if (compressed) {
        ...
        console.log(
          'Loaded data size: ' + compressedLength + ' (uncompressed: ' + decompressed.length + ')'
        );
        return obj;
      }
    } else {
  ...
  var loadFromLocalStorage = function (name) {
    if (localStorage) {
      ...
      if (compressed) {
        // Check for corrupted legacy data and clear it
        if (compressed.indexOf('DICT{') === 0) {
          console.warn('🗑️ Found corrupted legacy data for:', name, '- clearing it');
        ...
        try {
          ...
          console.warn('🗑️ Failed to parse data for:', name, '- clearing corrupted data');
          localStorage.removeItem(name);
          return null;
        }
      }
  ...
  // Add the missing ZERO_PAD function
  ...
  // Function to clear all corrupted legacy data on startup
  var clearCorruptedData = function () {
    if (!localStorage) return 0;

    var cleared = 0;
    var keys = [];

    // Collect all keys first (to avoid modifying localStorage while iterating)
    ...
    keys.forEach(function (key) {
      try {
        ...
        if (data && data.indexOf('DICT{') === 0) {
          console.log('🗑️ Clearing corrupted data:', key);
          localStorage.removeItem(key);
          cleared++;
        }
      } catch (e) {
        console.log('🗑️ Clearing unreadable data:', key);
        localStorage.removeItem(key);
        cleared++;
      }
    });
    ...
    if (cleared > 0) {
      console.log('🧹 Cleared', cleared, 'corrupted save entries');
    }
  ...
  Gui.clearCorruptedData = clearCorruptedData;

  // Make cleanup function available globally for manual use
  window.clearAllCorruptedSaveData = clearCorruptedData;

  // Automatically clear corrupted data on load
  console.log('🚀 SAVE/LOAD SYSTEM v999 - NO COMPRESSION MODE ACTIVE');
  console.log('🧹 Starting automatic cleanup of corrupted save data...');
  var cleared = clearCorruptedData();
  if (cleared > 0) {
    console.log(
      '✅ Cleanup complete! Cleared',
      cleared,
      'corrupted entries. Save/load should now work properly.'
    );
  } else {
    console.log('✅ No corrupted data found. Save/load system ready.');
  }
})();
