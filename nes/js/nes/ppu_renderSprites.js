...
PpuRenderSprites.prototype._renderSprite = function (
  spriteHeight,
  spritenum,
  startline,
  endline,
  ...
  for (var scanline = renderScanlineStart; scanline <= renderScanlineEnd; ++scanline) {
    ...
    if (spriteHeight === 8 /*!ppuControl1.spriteSize*/) {
      ppuAddress =
        patternnum * 16 +
        ((flipVert ? 7 - absSy : absSy) & 0x7) +
        ((this.ppu.control1 & 0x8) > 0 /*ppuControl1.spritePatternTableAddress*/ ? 0x1000 : 0);
    } // big sprites - if sprite num is even, use 0x0 else use 0x1000
    else {
      ...
      if (!topsprite) {
        ...
        if (flipVert) ppuAddress += 15 - scanline + spritey;
        else ppuAddress += 8 + absSy;
      } else {
        if (flipVert) ppuAddress += 23 - scanline + spritey;
        else ppuAddress += absSy;
      }
    }
    ...
    for (x = 0; x < 8; ++x) {
      ...
      if (paletteindex > 0) {
        ...
        this.ppu.mainboard.renderBuffer.renderSpritePixel(
          spritenum,
          behindBackground,
          absx,
          scanline,
          TYPED_ARRAY_GET_INT32(this.ppu.paletteTables[1], paletteindex & 0xf) | 0
        );
      }
...
PpuRenderSprites.prototype.renderTo = function (startTicks, endTicks) {
  ...
  var nextSpriteEval =
    startTicks - ticksIntoCurrentLine + spriteEvaluationStart * MASTER_CYCLES_PER_PPU;
  var startline = 0;
  var endline = 0;
