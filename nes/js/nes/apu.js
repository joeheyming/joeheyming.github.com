...
var Apu = function (mainboard) {
  ...
  // Mobile-optimized buffer sizes - more aggressive for Samsung/Android
  var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
    navigator.userAgent
  );
  var isAndroid = /Android/i.test(navigator.userAgent);

  // Samsung Galaxy and other Android devices need much larger buffers
  if (isAndroid) {
    this._outBufferSize = 16384; // 4x larger for Android devices
  } else if (isMobile) {
    this._outBufferSize = 8192; // 2x larger for other mobile
  } else {
    this._outBufferSize = 4096; // Desktop default
  }
...
Apu.prototype._addBuffer = function () {
  var buffer = new ApuOutputBuffer(
    this._renderer.createBuffer(this._outBufferSize),
    this._outBufferSize,
    this._soundRate
  );
