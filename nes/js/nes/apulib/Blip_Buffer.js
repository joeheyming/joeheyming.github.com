...
(function () {
  ...
  Blip_Impulse_.prototype.init = function (imps, w, r, fb) {
    ...
    this.impulse = new Uint16Array(
      this.impulses.buffer,
      this.width * this.res * 2 * (this.fine_bits ? 2 : 1) * 2
    );
    this.offset = 0;
  };
  ...
  Blip_Impulse_.prototype.treble_eq = function (new_eq) {
    if (
      !this.generate &&
      new_eq.treble === this.eq.treble &&
      new_eq.cutoff === this.eq.cutoff &&
      new_eq.sample_rate === this.eq.sample_rate
    ) {
    ...
    for (var i = size; i--; ) {
      ...
      var c =
        pow_a_n * rolloff * Math.cos((n_harm - 1.0) * angle) -
