...
var ppu = function (mainboard) {
  ...
  this.nameTables = [];
  for (var i = 0; i < 4; ++i) this.nameTables.push(new Int32Array(0x400));
...
ppu.prototype.resetVariables = function (cold) {
  if (cold) this.control1 = this.control2 = this.status = 0;
  else this.control1 &= 0x7f; // ppuControl1.vBlankNmi = false;
...
ppu.prototype.hookSyncEvents = function (synchroniser) {
  var that = this;
  this._clockSkipEventId = synchroniser.addEvent(
    'ppu clockskip',
    this.getMasterTicksTillClockSkip(),
    function () {
      that._eventClockskip();
    }
  );
  this._vblankClearEventId = synchroniser.addEvent(
    'ppu vblank clear',
...
ppu.prototype._eventClockskip = function () {
  ...
  if (
    this.isOddFrame &&
    (this.control2 & 0x8) > 0 /*ppuControl2.backgroundSwitch*/ &&
...
ppu.prototype._eventNmiTrigger = function (eventTime) {
  if (
    (this.control1 & 0x80) > 0 /* ppuControl1.vBlankNmi*/ &&
    (this.status & 0x80) > 0 /* ppuStatus.vBlank*/
  ) {
...
ppu.prototype.ticksToScreenCoordinates = function (tickCount) {
  ...
  screenPos.y =
    (Math.floor(tickCount / PPU_TICKS_PER_SCANLINE) - COLOUR_ENCODING_VBLANK_SCANLINES - 1) | 0;
  return screenPos;
};

ppu.prototype.screenCoordinatesToTicks = function (x, y) {
  return (
    x * MASTER_CYCLES_PER_PPU +
    (y + COLOUR_ENCODING_VBLANK_SCANLINES + 1) * MASTER_CYCLES_PER_SCANLINE
  );
};
  ...
  if (method !== this.mirroringMethod) {
    ...
    switch (this.mirroringMethod) {
      ...
      case PPU_MIRRORING_FOURSCREEN: // no mirroring done, requires an extra 0x800 of memory kept on cart
        for (var i = 0; i < 4; ++i) this.nameTablesMap[i] = i;
        //name = 'four screen';
        break;
      case PPU_MIRRORING_SINGLESCREEN_NT0:
        for (var j = 0; j < 4; ++j) this.nameTablesMap[j] = 0;
        //name = 'single 0';
        break;
      case PPU_MIRRORING_SINGLESCREEN_NT1:
        for (var k = 0; k < 4; ++k) this.nameTablesMap[k] = 1;
        //name = 'single 1';
        break;
    }
...
ppu.prototype.handleSpriteTransfer = function () {
  ...
  if (this.doSpriteTransferAfterNextCpuInstruction) {
    ...
    if (this.isOddFrame) this._sync.advanceCpuMTC(1 * COLOUR_ENCODING_MTC_PER_CPU);
  }
};
...
ppu.prototype._writeTo2000 = function (offset, data) {
  ...
  if ((data & 0x80) === 0) {
    ...
    if (ticksTillSet <= -MASTER_CYCLES_PER_PPU * 2 && ticksTillSet >= -MASTER_CYCLES_PER_PPU * 4) {
      ...
      ticksTillSet >= -MASTER_CYCLES_PER_PPU * 1 &&
      ticksTillSet <= MASTER_CYCLES_PER_PPU * 1
    ) {
  ...
  if (spriteScreenAddressChanged) {
    this.mainboard.cart.memoryMapper.spriteScreenEnabledUpdate(
      (this.control1 & 0x8) > 0,
      (this.control1 & 0x10) > 0
    );
...
ppu.prototype._readFromRegister2007 = function () {
  ...
  if (!this.isRendering(this._sync.getCpuMTC(), true)) {
    newAddress =
      (this.ppuReadAddress + ((this.control1 & 0x4) > 0 /*ppuControl1.verticalWrite*/ ? 32 : 1)) &
      0xffff;
    this.updatePPUReadAddress(newAddress, true);

    if ((bufferedaddress & 0xff00) === 0x3f00) {
      //IS_INT_BETWEEN( bufferedaddress, 0x3F00, 0x4000 ) )
...
ppu.prototype.readFromRegister = function (offset) {
  ...
  switch (
    offset // offset is 0x2000 -> 0x2008
  ) {
    case 0x2:
  ...
  if ((offset & 0x2000) === 0) {
    ...
    if ((offset & 0x3f00) === 0x3f00) {
      ...
      if (this.mainboard.cart.memoryMapper.nameTableWrite) {
        this.mainboard.cart.memoryMapper.nameTableWrite(
          this.nameTables,
          pageid,
          offset & 0x3ff,
          data
...
ppu.prototype.read8 = function (offset, renderingSprites, readType) {
  ...
  if ((offset & 0x2000) === 0) {
    ...
    // pattern tables
    return (
      this.mainboard.cart.memoryMapper.read8ChrRom(offset & 0x1fff, renderingSprites, readType) | 0
    );
  } else {
    if ((offset & 0x3f00) === 0x3f00) {
      // IS_INT_BETWEEN( offset, 0x3F00, 0x4000 )
      ...
      if (this._useMapperNameTableRead) {
        return (
          this.mainboard.cart.memoryMapper.nameTableRead(this.nameTables, pageid, offset & 0x3ff) |
          0
        );
      } else {
...
ppu.prototype.synchronise = function (startTicks, endTicks) {
  if (this.isRenderingEnabled()) {
    ...
    this.ppuReadAddress = this._ppuRenderBg.renderTo(
      startTicks,
      endTicks,
      this.ppuReadAddress,
      this.ppuLatchAddress
    );
  }
};
...
ppu.prototype.onEndFrame = function () {
  ...
  if (this.forceNmi || (!this.suppressNmi && (this.control1 & 0x80) > 0)) {
    Nes.Trace.writeLine(
      Nes.trace_ppu,
      'this.mainboard.cpu.nonMaskableInterrupt: ' +
        (COLOUR_ENCODING_FRAME_MTC + MASTER_CYCLES_PER_PPU)
    );
...
ppu.prototype.readNameTable = function (address, readType) {
  ...
  if (this._useMapperNameTableRead) {
    return (
      this.mainboard.cart.memoryMapper.nameTableRead(
        this.nameTables,
        pageid,
        address & 0x3ff,
        readType
...
ppu.prototype.formatStatusString = function () {
  ...
  str +=
    ' S:' +
    ZERO_PAD_HEX(this.status, 2) +
    ' C1:' +
    ZERO_PAD_HEX(this.control1, 2) +
    ' C2:' +
    ZERO_PAD_HEX(this.control2, 2);
  return str;
};
