...
cartridge.prototype._loadData = function (name, binaryString, completeCallback) {
  ...
  try {
    ...
    that._sramEnabled = sramEnabled;

    var mirroringMethod = 0;
    if (fourScreenRamLayout) mirroringMethod = PPU_MIRRORING_FOURSCREEN;
    else if (!horizontalMirroring) mirroringMethod = PPU_MIRRORING_VERTICAL;
    else mirroringMethod = PPU_MIRRORING_HORIZONTAL;
...
cartridge.prototype.saveSram = function () {
  if (this._sramEnabled && this.memoryMapper && this.memoryMapper.sram) {
    var sramData = Nes.uintArrayToString(this.memoryMapper.sram);
...
cartridge.prototype.autoSaveSram = function () {
  // Auto-save SRAM every 30 seconds if it's enabled and has been modified
  if (this._sramEnabled && Date.now() - this._lastSramSave > 30000) {
    this.saveSram();
  }
};
