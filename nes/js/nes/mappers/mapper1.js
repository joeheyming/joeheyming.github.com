...
mapper1.prototype.write8PrgRom = function (offset, data) {
  ...
  // Mobile optimization: reduce timing precision for better performance
  var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
    navigator.userAgent
  );
  var currTime = this.mainboard.synchroniser.getCpuMTC();
  var minCycles = isMobile ? 1 : 2; // Reduce timing precision on mobile
  var minTime = this.lastWriteMTC + COLOUR_ENCODING_MTC_PER_CPU * minCycles;
  ...
  if (this.count >= 5) {
    // Mobile optimization: reduce synchronise calls for better performance
    var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
      navigator.userAgent
    );
    if (!isMobile || this.mainboard.synchroniser.getCpuMTC() % 4 === 0) {
...
mapper1.prototype.read8SRam = function (offset) {
  if (this.wRamEnabled) return Nes.basemapper.prototype.read8SRam.call(this, offset);
  else return 0; // TODO: supposed to be open bus
};
