...
mapper64.prototype.calculateNextA12Raise = function (cpuMTC) {
  ...
  if (this.mRenderingEnabled) {
    ...
    // pixelEvent = 340;//324;
    // firstScanline = -1;
    // } //if ( this.mScreenAddress && !this.mSpriteAddress )
    else {
      pixelEvent = 9; // 324; // 9;
      firstScanline = 0; // -1;
    }
  ...
  if (!this._cpuCycleMode) {
    ...
    if (this.interruptsEnabled) {
      ...
      if (nextDecrement === -1) {
        ...
        newEvent =
          nextDecrement + count * (COLOUR_ENCODING_MTC_PER_CPU * 4) + COLOUR_ENCODING_MTC_PER_CPU; // RAMBO-1: The actual interrupt triggers one M2 cycle later than one would naively expect.
      }
    }
    this.mainboard.synchroniser.changeEventTime(this._irqEventId, newEvent);
  }
  ...
  if (!this._cpuCycleMode) {
	...
	*/
    ...
    if (startMtc >= 0) {
      for (
        var mtc = startMtc;
        mtc <= Math.min(this._A12UpperLimit, endTicks);
        mtc += MASTER_CYCLES_PER_SCANLINE
      ) {
