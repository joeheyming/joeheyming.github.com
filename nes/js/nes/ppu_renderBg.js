...
PpuRenderBg.prototype.onControl1Change = function (control1) {
  this._bgTableAddress =
    (control1 & 0x10) > 0 /*ppuControl1.screenPatternTableAddress*/ ? 0x1000 : 0;
};
...
PpuRenderBg.prototype._renderTile = function (ppuReadAddress, tilenum, posy, clippingEnabled) {
  ...
  var attributeByte = this.ppu.readNameTable(
    0x23c0 | (ppuReadAddress & 0x0c00) | ((vtile & 0x1c) << 1) | ((htile >> 2) & 0x7),
    1
  );
  ...
  for (; x <= endPixel; ++x) {
    ...
    if (x >= startXRendering && x < SCREEN_WIDTH) {
      if (paletteIndex > 0) {
        ...
        if (
          this._renderBuffer.renderPixel(
            x,
            renderScanline,
            TYPED_ARRAY_GET_INT32(this.ppu.paletteTables[0], paletteIndex & 0xf) | 0
          )
        ) {
          ...
          if (!this._spriteZeroHit) {
            ...
            Nes.Trace.writeLine(
              Nes.trace_ppu,
              '[' +
                this.ppu.frameCounter +
                '] PPU sprite hit scheduled for @ ' +
                x +
                'x' +
                renderScanline +
                ' (' +
            ...
            this.ppu.mainboard.synchroniser.changeEventTime(
              this.ppu._spriteZeroEventId,
              triggerTime
            );
          }
        }
...
PpuRenderBg.prototype.renderTo = function (startTicks, endTicks, ppuReadAddress, ppuLatchAddress) {
  ...
  ticksAtFirstRenderingScanline =
    ticksAtFirstScanline -
    MASTER_CYCLES_PER_SCANLINE +
    SecondLastTileReloadTime * MASTER_CYCLES_PER_PPU;
  ticksAtFirstRenderingScanlineEnd = ticksAtFirstRenderingScanline + MASTER_CYCLES_PER_SCANLINE; // ( 34 * 8 * MASTER_CYCLES_PER_PPU );

  while (
    ticksAtFirstRenderingScanlineEnd < startTicks ||
    ticksAtFirstRenderingScanline < backgroundRenderingStart
  ) {
  ...
  scanline =
    Math.floor(
      (ticksAtFirstRenderingScanline - backgroundRenderingStart) / MASTER_CYCLES_PER_SCANLINE
    ) | 0;
  ...
  while (scanlineStart <= endTicks) {
    ...
    if (
      incrementYTime < backgroundRenderingEnd &&
      incrementYTime > startTicks &&
      incrementYTime <= endTicks
    ) {
