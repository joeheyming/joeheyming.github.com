...
| `google-auth.js`   | GIS token client; imports **`site-config.js`**. OAuth scope **`drive.file`**. **`waitForGoogle()`** (GIS script with `defer`). `localStorage`: `google-db.oauthAccessToken` / `google-db.oauthExpiresAt`; **`LS_SPREADSHEET_ID`**, **`initGoogleAuth()`** |
...
- Load `https://accounts.google.com/gsi/client` before calling `initGoogleAuth()` (no arguments; reads **`clientId`** from `site-config.js`). If the script uses `defer`, **`await waitForGoogle()`** from **`google-auth.js`** first.
