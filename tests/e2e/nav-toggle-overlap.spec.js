...
for (const app of APPS) {
  for (const vp of VIEWPORTS) {
    test(`${app} — no nav-toggle overlap at ${vp.name}`, async ({ page }) => {
      ...
      await page.waitForTimeout(600); // allow nav.js to inject

      const btn = page.locator('.heyming-nav-toggle');
      ...
      const overlaps = candidates.filter((c) => {
        ...
        if (
          ...
          return false;
        // Skip the toggle's own text
        if (c.text.includes('Menu') || c.text.includes('☰')) return false;
        // Full-width block elements with centered/right text are false-positives:
        // the toggle visually covers empty space, not the actual text.
      ...
      expect(
        overlaps,
        `Nav toggle overlaps at ${app} [${vp.name}]: ${JSON.stringify(overlaps)}`
      ).toHaveLength(0);
    });
  }
}
